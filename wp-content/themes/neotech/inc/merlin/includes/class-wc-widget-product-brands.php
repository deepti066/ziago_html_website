 ...
 * @extends WC_Widget
 */
class Neotech_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'neotech');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'neotech');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'neotech'),
				'label' => esc_html__('Title', 'neotech'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'neotech'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'neotech'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'neotech'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'neotech'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'neotech'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'neotech'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'neotech'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Neotech_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'neotech');
			...
			echo '<ul class="product-brands" id="neotech-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Neotech_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'neotech'),
		'singular_name'              => esc_html__('Brands', 'neotech'),
		'menu_name'                  => esc_html__('Brands', 'neotech'),
		'all_items'                  => esc_html__('All Brands', 'neotech'),
		'parent_item'                => esc_html__('Parent Brand', 'neotech'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'neotech'),
		'new_item_name'              => esc_html__('New Brand Name', 'neotech'),
		'add_new_item'               => esc_html__('Add New Brands', 'neotech'),
		'edit_item'                  => esc_html__('Edit Brand', 'neotech'),
		'update_item'                => esc_html__('Update Brand', 'neotech'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'neotech'),
		'search_items'               => esc_html__('Search Brands', 'neotech'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'neotech'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'neotech'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
