...
class Neotech_Content_Service {
	...
	protected function get_query_results() {
		 ...
		 */
		return apply_filters( 'neotech_shortcode_service_query_results', $results, $this );
	}
	...
	protected function service_loop() {
		...
		if ( $service && $service->ids ) {
			...
			neotech_setup_service_loop($setup_args);

            // echo '<pre>'; print_r(); echo '</pre>';

			$original_post = $GLOBALS['post'];

			neotech_service_loop_start();

			if ( neotech_get_service_loop_prop( 'total' ) ) {
			...
			neotech_service_loop_end();

			if ( neotech_string_to_bool( $this->attributes['paginate'] ) ) {
				$args = array(
					'total'   => neotech_get_service_loop_prop( 'total_pages' ),
					'current' => neotech_get_service_loop_prop( 'current_page' ),
			...
			if (isset($this->attributes['enable_carousel']) && $this->attributes['enable_carousel'] === 'yes') {
                neotech_set_service_loop_prop('enable_carousel', false);
			...
			wp_reset_postdata();
			neotech_service_reset_loop();
		} else {
			echo esc_attr(apply_filters('neotech_service_nodata_found', __('No data was founded', 'neotech')));
		}
