...
if ($related_products) :
    ...
    $settings = neotech_get_settings_product_caroseul();
    ...
    <section class="related products elementor-element">
        ...
        <?php
        $heading = apply_filters('woocommerce_product_related_products_heading', __('Related Products', 'neotech'));

        if ($heading) :
            ?>
            <h2><?php echo esc_html($heading); ?></h2>
            <span><?php _e('Follow the most popular trends and get exclusive items from shop', 'neotech'); ?></span>
        <?php endif; ?>
        ...
        if (!neotech_is_elementor_activated()) {
            ?>
            <style>
                .related .neotech-products.products.elementor-grid {
                    display: flex;
                }
            </style>
            <?php
